"""LLM provider config: one `LLM_PROVIDER` toggle over tau_ai.env (all OpenAI-compatible).
...
class WebHermesConfig:
    ...
    def to_provider_config(self) -> OpenAICompatibleConfig | None:
        """tau_ai provider config, or None when keyless (fake/dry-run)."""
        if not self.api_key:
            return None
...
def _load_dotenv() -> None:
    """Load .env if present (same search order as tau_coding.cli; no new dep)."""
    candidates = [
        Path.cwd() / ".env",
