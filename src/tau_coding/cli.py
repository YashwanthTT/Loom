...
This is a stripped-down replacement for the full Tau CLI.

What it teaches (maps to Pi architecture):
  tau_coding/cli.py  -> simple frontend (this file)
  tau_agent/harness.py + loop.py -> reusable brain
  tau_ai/fake.py or openai_compatible.py -> provider layer
  tau_coding/tools.py -> filesystem/shell tools
...
import anyio
import typer
from tau_ai.env import DEFAULT_OPENCODE_BASE_URL, DEFAULT_OPENCODE_MODEL
...
def _load_dotenv() -> None:
    ...
    # Search: cwd/.env, repo-root/.env, ~/.tau/.env
    candidates = [
        Path.cwd() / ".env",
...
from tau_agent.events import AgentEvent, MessageEndEvent, MessageUpdateEvent, ToolExecutionEndEvent
from tau_coding.tools import create_coding_tools
from tau_coding.webhermes.tui import app as webhermes_app

app = typer.Typer(
    name="tau-minimal",
    help="Minimal Tau harness for learning - harness + simple CLI + tools.",
    add_completion=False,
)
app.add_typer(webhermes_app, name="webhermes")


@app.callback(invoke_without_command=True)
def _default(ctx: typer.Context) -> None:
    """No command ⇒ launch the WebHermes TUI."""
    if ctx.invoked_subcommand is None:
        from tau_coding.webhermes.app import run_app
...
def _resolve_provider(model: str | None):
    ...
    if use_opencode and opencode_key:
        ...
        # Model priority: explicit arg > OPENCODE_MODEL > ZEN_MODEL > TAU_MODEL > Go default
        raw_model = model or os.environ.get("OPENCODE_MODEL") or os.environ.get("ZEN_MODEL") or os.environ.get("TAU_MODEL") or DEFAULT_OPENCODE_MODEL
        ...
        chosen_model = model or os.environ.get("TAU_MODEL", "gpt-4o-mini")
        from tau_ai.env import OpenAICompatibleConfig
        from tau_ai.openai_compatible import OpenAICompatibleProvider
        ...
        # OpenAICompatibleProvider in tau_ai expects provider config object,
        ...
        raw = model or os.environ.get("OPENCODE_MODEL") or os.environ.get("TAU_MODEL", "gpt-4o-mini")
    ...
    if opencode_key is not None or "OPENCODE_MODEL" in os.environ:
        help_text = f"Hello from minimal Tau! Set OPENCODE_API_KEY and OPENCODE_MODEL={DEFAULT_OPENCODE_MODEL} for real Go model calls (or OPENAI_API_KEY for OpenAI)."
    else:
        help_text = "Hello from minimal Tau! Set OPENCODE_API_KEY for real Go model calls (or OPENAI_API_KEY)."
...
def main(
    ...
    # REPL for learning
    typer.echo(f"Minimal Tau REPL (model={model or 'auto'}, cwd={cwd})")
