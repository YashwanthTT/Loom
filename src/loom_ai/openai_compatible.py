...
from loom_agent.tools import AgentTool, ToolCall
from loom_agent.types import JSONValue
from loom_ai._provider_events import (
    ProviderErrorEvent,
    ProviderEvent,
...
from loom_ai.events import AssistantMessageEvent
from loom_ai.http import create_async_client
from loom_ai.http_errors import provider_http_error_message
from loom_ai.openai_cache import is_direct_openai_url, openai_prompt_cache_key
from loom_ai.provider import CancellationToken
from loom_ai.retry import provider_retry_event, retry_delay_seconds, wait_for_retry
from loom_ai.stream import canonicalize_provider_stream
from loom_ai.tool_call_ids import portable_tool_call_id

# Models that reject function tools + reasoning_effort on /chat/completions and
# must use the /v1/responses endpoint instead. Includes OpenCode Go
# responses-path models (https://opencode.ai/docs/go#endpoints).
_RESPONSES_ONLY_PREFIXES: tuple[str, ...] = ("gpt-5.5", "gpt-5.4", "gpt-5.6-luna", "grok-4")


def _use_responses_api(model: str) -> bool:
    """Return whether ``model`` must be served over the Responses API."""
    normalized = model.strip().lower()
    # Muse Spark family (opencode) is Responses-only on Zen/Go
    # e.g. muse-spark-1.3-contributor, muse-spark-1.2-contributor-free
    ...
    def _stream(
        ...
        *,
        ...
        async def iterator() -> AsyncIterator[ProviderEvent]:
            ...
            _apply_opencode_go_headers(headers, self._config.base_url, session_id)
...
def _apply_opencode_go_headers(
    headers: dict[str, str],
    base_url: str,
    session_id: str | None,
) -> None:
    """Add OpenCode Go's required User-Agent + session headers.

    https://opencode.ai/docs/go requires coding-agent traffic to identify
    itself (not a generic SDK name) and send a stable per-conversation
    ``x-opencode-session`` for routing/prompt caching.
    """
    if "/zen/go" not in base_url:
        return
    if not any(key.casefold() == "user-agent" for key in headers):
        from loom_ai.env import TAU_USER_AGENT

        headers["User-Agent"] = TAU_USER_AGENT
    if session_id is not None and not any(
        key.casefold() == "x-opencode-session" for key in headers
    ):
